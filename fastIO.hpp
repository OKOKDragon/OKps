...
namespace OKps::file
{
	...
	class reader : public OKps::basic
	{
	private:
	...
	public:
		...
		// 为了模仿标准库的行为，read失败会将读指针read_position改为-1，必须调用clear来重置
	...
	class writter : public OKps::basic
	{
	private:
	...
	public:
		...
		void open(const std::string & _route, std::ios_base::openmode _mode = std::ios_base::out, int _prot = std::ios_base::_Default_open_prot, bool resize = false, uintmax_t guess_size = 0);
		void open(const char * _route, std::ios_base::openmode _mode = std::ios_base::out, int _prot = std::ios_base::_Default_open_prot, bool resize = false, uintmax_t guess_size = 0);
		void open(const std::wstring & _route, std::ios_base::openmode _mode = std::ios_base::out, int _prot = std::ios_base::_Default_open_prot, bool resize = false, uintmax_t guess_size = 0);
		void open(const wchar_t * _route, std::ios_base::openmode _mode = std::ios_base::out, int _prot = std::ios_base::_Default_open_prot, bool resize = false, uintmax_t guess_size = 0);
		void open(const std::filesystem::path & _route, std::ios_base::openmode _mode = std::ios_base::out, int _prot = std::ios_base::_Default_open_prot, bool resize = false, uintmax_t guess_size = 0);

	private:
		template <typename string_type>
		void INNER_open(string_type _route, std::ios_base::openmode _mode = std::ios_base::in, int _prot = std::ios_base::_Default_open_prot, bool resize = false, uintmax_t guess_size = 0);
	...
	public:
		...
		std::streampos tellp() const noexcept;
		...
		void resize_buffer(const std::size_t size);
	...
	public:
		// 自动调整缓冲区大小
		void auto_resizer(const uintmax_t size_guess);
	...
	class sequential_writter : public OKps::basic
	{
	private:
		std::ofstream file;			 // 文件
		std::filesystem::path route; // 文件路径
	...
	public:
		sequential_writter(std::size_t size = 64 * 1024);

		sequential_writter(const sequential_writter &) = delete;
		void operator=(const sequential_writter &) = delete;
		sequential_writter(sequential_writter &&) = delete;
		void operator=(sequential_writter &&) = delete;

		// 如果析构时文件没有关闭，将自动关闭
		// 这可能会引发未知问题
		~sequential_writter();

		void open(const std::string & _route, std::ios_base::openmode _mode = std::ios_base::out | std::ios_base::app, int _prot = std::ios_base::_Default_open_prot, bool resize = false, uintmax_t guess_size = 0);
		void open(const char * _route, std::ios_base::openmode _mode = std::ios_base::out | std::ios_base::app, int _prot = std::ios_base::_Default_open_prot, bool resize = false, uintmax_t guess_size = 0);
		void open(const std::wstring & _route, std::ios_base::openmode _mode = std::ios_base::out | std::ios_base::app, int _prot = std::ios_base::_Default_open_prot, bool resize = false, uintmax_t guess_size = 0);
		void open(const wchar_t * _route, std::ios_base::openmode _mode = std::ios_base::out | std::ios_base::app, int _prot = std::ios_base::_Default_open_prot, bool resize = false, uintmax_t guess_size = 0);
		void open(const std::filesystem::path & _route, std::ios_base::openmode _mode = std::ios_base::out | std::ios_base::app, int _prot = std::ios_base::_Default_open_prot, bool resize = false, uintmax_t guess_size = 0);
	...
	public:
		// 自动调整缓冲区大小
		void auto_resizer(uintmax_t size_guess);
	};
}
