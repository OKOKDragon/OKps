...
namespace OKps
{
	...
	field_stream::field_stream(std::string && data)
	...
	field_stream::field_stream(std::vector<std::byte> const & data)
		...
		:MEMBER_data()
	{
		this->MEMBER_data.resize(data.size());
		for (std::size_t i = 0;i < this->MEMBER_data.size();++i)
		{
			this->MEMBER_data[i] = value_cast<char>(data[i]);
		}
	}

	std::string const & field_stream::raw_string()const noexcept
	{
		return this->MEMBER_data;
	}
	std::string & field_stream::raw_string()noexcept
	{
		return this->MEMBER_data;
	}
...
and noexcept(std::declval<std::string>().resize(std::declval<std::string>().size())))
	{
		std::string result;
		result.resize(this->MEMBER_data.size() + sizeof(std::size_t));
		std::size_t const temp_length = this->MEMBER_data.size();
	...
	{
		return value_cast<std::vector<std::byte>>(this->MEMBER_data);
	}
...
and noexcept(std::declval<std::vector<std::byte>>().resize(std::declval<std::string>().size())))
	{
		...
		for (std::size_t i = 0; i < this->MEMBER_data.size(); ++i)
		{
			result[i + sizeof(std::size_t)] = value_cast<std::byte>(this->MEMBER_data[i]);
		}
		return result;
	}
	...
	std::vector<field_stream> field_stream::parse(std::vector<std::byte> const & data)
	{
		...
		while (true)
		{
			...
			result.push_back(field_stream(std::move(buffer)));
		}
		return result;
	}
		...
		noexcept(std::is_nothrow_copy_assignable_v<std::string>)
	{
		if (this != (&origin))
		{
			this->MEMBER_data = origin.MEMBER_data;
		}
	}
