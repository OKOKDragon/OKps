...
namespace OKps
{
    ...
    */
    class binary_tree : public basic
    {
        ...
        */
        ...
        public:
            ...
            void left_birth(data_type && data)
            {
                if (this->MEMBER_left)
                {
                    throw error(u8"此节点已经有左孩子，禁止重复初始化。", error::error_type::DEBUG_logic);
                }
                else
                {
                    // new新节点，传入此节点作为父亲
            ...
            void right_birth(data_type && data)
            {
                ...
                {
                    throw error(u8"此节点已经有右孩子，禁止重复初始化。", error::error_type::DEBUG_logic);
                }
                else
                {
                    // new新节点，传入此节点作为父亲
            ...
            */
            ...
            binary_tree<data_type>  split_tree()
            {
                ...
                if (not TEMP_parent)
                {
                    throw error(u8"此节点已经是根节点，无法拆分出子树", error::error_type::DEBUG_logic);
                }
                else
                {
                    // new空树，将根节点改为此节点
            ...
            void combine_left(binary_tree<data_type> && subtree)
            {
                if (this->MEMBER_left)
                {
                    throw error(u8"此节点已经有左孩子，无法合并子树", error::error_type::DEBUG_logic);
                }
                else
                {
            ...
            void combine_right(binary_tree<data_type> && subtree)
            {
                if (this->MEMBER_right)
                {
                    throw error(u8"此节点已经有右孩子，无法合并子树", error::error_type::DEBUG_logic);
                }
                else
                {
                    subtree.INNER_change_owner_tree(this->MEMBER_tree);
        ...
        static void swap_data(typename node::SPTR_node const & node1, typename node::SPTR_node const & node2)
        {
            ...
            // 输入空指针
            {
                throw error(u8"禁止输入空指针", error::error_type::DEBUG_bad_parameter);
            }
            else
            {
        ...
        */
        static void swap_tree(typename node::SPTR_node const & node1, typename node::SPTR_node const & node2)
        {
            ...
            */
            ...
            {
                throw error(u8"禁止输入的节点之间存在祖先关系", error::error_type::DEBUG_bad_parameter);
                return;
            }
    ...
    public:
        ...
        void root_birth(data_type && data)
        {
            if (this->MEMBER_root)
            {
                throw error(u8"根节点不空，禁止重复初始化。", error::error_type::DEBUG_logic);
            }
            else
            {
        ...
        //迭代器
        class itorator :public basic
        {
            friend class binary_tree;
        private:
            typename node::WPTR_node MEMBER_node;

            itorator(typename node::SPTR_node const & origin = typename node::SPTR_node())noexcept
                :MEMBER_node(origin)
            {
            }
        public:
        ...
        private:
            typename node::SPTR_node ERROR_invalid_node()const
            {
                ...
                if (not (temp_node))
                {
                    throw error(u8"此迭代器引用的是空节点，无法操作", error::error_type::DEBUG_invalid_object);
                }
                return temp_node;
            }
        public:
            itorator(itorator const & origin)noexcept
                :MEMBER_node(origin.MEMBER_node)
            {
            }
            void operator=(itorator const & origin)noexcept
            {
                this->MEMBER_node = origin.MEMBER_node;
            }
            itorator(itorator && origin)noexcept
                :MEMBER_node(std::move(origin.MEMBER_node))
            {
            }
            void operator=(itorator && origin)noexcept
            {
                this->MEMBER_node = std::move(origin.MEMBER_node);
            }
            bool operator==(itorator const & right)const
            {
            ...
            {
                auto temp_node = this->ERROR_invalid_node();
                return itorator(temp_node->left());
            }
            itorator const left()const
            {
                auto temp_node = this->ERROR_invalid_node();
                return itorator(temp_node->left());
            }
            ...
            {
                auto temp_node = this->ERROR_invalid_node();
                return itorator(temp_node->right());
            }
            itorator const right()const
            {
                auto temp_node = this->ERROR_invalid_node();
                return itorator(temp_node->right());
            }
            ...
            {
                auto temp_node = this->ERROR_invalid_node();
                return itorator(temp_node->parent());
            }
            itorator const parent()const
            {
                auto temp_node = this->ERROR_invalid_node();
                return itorator(temp_node->parent());
            }
            ...
            {
                auto temp_node = this->ERROR_invalid_node();
                return itorator(temp_node->root());
            }
            itorator const root()const
            {
                auto temp_node = this->ERROR_invalid_node();
                return itorator(temp_node->root());
            }
        ...
        private:
           ...
           // 深度
            ...
            using TYPE_list = std::vector<itorator>;
           // 遍历数组
        ...
        */
        static tree_information front_traversal(typename  node::SPTR_node const & root)
        {
            ...
            while (true)
            {
                ...
                {
                    // 迭代器指向的节点写入结果
                    result.push_back(itorator(_root));
                    count++;
                    if (_root->is_leaf())
                    {
                        leaf++;
    ...
    // 禁止输入空节点
        static tree_information level_traversal(typename  node::SPTR_node const & root)
        {
            ...
            while (not assistant.empty())
            {
                ...
                assistant.pop();
                result.push_back(itorator(temp));
                count++;
                if (temp->is_leaf())
                {
                    leaf++;
    ...
    public:
        ...
        如果找不到，返回空迭代器
        */
        itorator find(data_type const & data)
        {
            auto temp = this->behind_traversal();
        ...
        itorator const find(data_type const & data)const
        {
            auto temp = this->behind_traversal();
            for (std::size_t count = 0; count < temp.MEMBER_list.size(); count++)
            {
        ...
        static void swap_data(itorator & node1, itorator & node2)
        {
        ...
        输入的2个节点之间也不能有祖先关系
        */
        static void swap_tree(itorator & node1, itorator & node2)
        {
            auto temp_node1 = node1.ERROR_invalid_node();
            auto temp_node2 = node2.ERROR_invalid_node();
        ...
        注意空迭代器是无法使用的
        */
        itorator root()noexcept
        {
            return itorator(this->MEMBER_root);
        }
        itorator const root()const noexcept
        {
            return itorator(this->MEMBER_root);
        }


        static const tree_information level_traversal(itorator const & root)
        {
            auto temp_root = root.ERROR_invalid_node();

            return binary_tree::level_traversal(temp_root);
        }
        static  tree_information level_traversal(itorator & root)
        {
            auto temp_root = root.ERROR_invalid_node();

            return binary_tree::level_traversal(temp_root);
        }
        static  tree_information behind_traversal(itorator & root)
        {
            auto temp_root = root.ERROR_invalid_node();

            return binary_tree::behind_traversal(temp_root);
        }
        static const tree_information behind_traversal(itorator const & root)
        {
            auto temp_root = root.ERROR_invalid_node();

            return binary_tree::behind_traversal(temp_root);
        }
        static const tree_information middle_traversal(itorator const & root)
        {
            auto temp_root = root.ERROR_invalid_node();

            return binary_tree::middle_traversal(temp_root);
        }
        static tree_information middle_traversal(itorator & root)
        {
            auto temp_root = root.ERROR_invalid_node();

            return binary_tree::middle_traversal(temp_root);
        }
        static const tree_information front_traversal(itorator const & root)
        {
            auto temp_root = root.ERROR_invalid_node();

            return binary_tree::front_traversal(temp_root);
        }
        static tree_information front_traversal(itorator & root)
        {
            auto temp_root = root.ERROR_invalid_node();
