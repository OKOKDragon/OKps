...
namespace OKps
{
    ...
    const bits::TYPE_pos bits::INNER_trans_pos(const std::size_t ARG_pos) const noexcept
    {
        ...
        {
            const bits::TYPE_pos result(ARG_pos / DATA_bit_op_len, ARG_pos % DATA_bit_op_len);
            return result;
        }
        // 真实位置在第0块内
        if (ARG_pos + this->MEMBER_front_pos < DATA_bit_op_len)
        {
        ...
        {
            std::size_t TEMP_sft = ARG_pos - (DATA_bit_op_len - this->MEMBER_front_pos); // ARG_pos相对于第1块第0位的偏移量
            const bits::TYPE_pos result(TEMP_sft / DATA_bit_op_len + 1, TEMP_sft % DATA_bit_op_len);
            return result;
        }
    }
    TYPE_bit_op::reference bits::INNER_unsafe_at(const std::size_t ARG_pos) noexcept
    {
    ...
    {
        throw error(u8"访问越界", error::error_type::DEBUG_overstep);
    }
    ...
    void bits::ERROR_empty() const
    {
        ...
        {
            throw error(u8"容器为空", error::error_type::DEBUG_logic);
        }
    }
    ...
    {
        throw error(u8"字符不能表示二进制位", error::error_type::DEBUG_bad_parameter);
    }
    bits::bits() noexcept
        ...
        : MEMBER_front_pos(0), MEMBER_size(ARG_size)
    {
        ...
        {
            this->MEMBER_data.push_back(TYPE_bit_op());
        }
    }
    ...
    TYPE_bit_op::reference bits::operator[](const std::size_t ARG_pos)
    {
        return this->at(ARG_pos);
    }
    TYPE_bit_op::reference bits::at(const std::size_t ARG_pos)
    {
    ...
    TYPE_bit_op::reference bits::back()
    {
        this->ERROR_empty();
        return this->INNER_unsafe_at(this->MEMBER_size - 1);
    }
    TYPE_bit_op::reference bits::front()
    {
        this->ERROR_empty();
    ...
    void bits::push_front(const bool ARG_value)
    {
        if (this->MEMBER_front_pos == 0)
        {
            this->MEMBER_data.push_front(TYPE_bit_op());
            this->MEMBER_front_pos = DATA_bit_op_len - 1;
        }
        else
        {
    ...
    void bits::push_front(const bits & ARG_input)
    {
        ...
        {
            for (std::size_t TEMP_count = 0; TEMP_count < (ARG_input.MEMBER_size - this->MEMBER_front_pos) / DATA_bit_op_len + 1; TEMP_count++)
            {
                this->MEMBER_data.push_front(TYPE_bit_op());
            }
            this->MEMBER_front_pos = DATA_bit_op_len - ((ARG_input.MEMBER_size - this->MEMBER_front_pos) % DATA_bit_op_len);
        }
    ...
    bool bits::pop_front()
    {
        ...
        if (this->MEMBER_front_pos + 1 == DATA_bit_op_len)
        {
            this->MEMBER_front_pos = 0;
    ...
    void bits::pop_front(const std::size_t ARG_length)
    {
        ...
        {
            if (ARG_length + this->MEMBER_front_pos < DATA_bit_op_len)
            {
                this->MEMBER_front_pos = ARG_length + this->MEMBER_front_pos;
            }
            else
            {
                for (std::size_t TEMP_count = 0; TEMP_count < (ARG_length - (DATA_bit_op_len - this->MEMBER_front_pos)) / DATA_bit_op_len + 1; TEMP_count++)
                {
                    this->MEMBER_data.pop_front();
                }
                this->MEMBER_front_pos = (ARG_length - (DATA_bit_op_len - this->MEMBER_front_pos)) % DATA_bit_op_len;
            }
        }
    }
    ...
    void bits::ERROR_diff_len(const bits & ARG_right) const
    {
        if (this->MEMBER_size != ARG_right.MEMBER_size)
        {
            throw error(u8"位操作的2个操作数长度不同", error::error_type::DEBUG_bad_parameter);
        }
    }
