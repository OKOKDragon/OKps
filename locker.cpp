...
namespace OKps
{
            ...
            else
            {
                ...
                */
                if (this->MEMBER_thread_id == std::this_thread::get_id())
                {
                    ...
                    */
                    throw error(u8"当前线程申请锁，但此锁已经被该线程占用，重复申请会导致死锁", error::error_type::DEBUG_thread);
                }
                else
                {
                    /*
        ...
        if (this->MEMBER_lock.load() == true)
        {
            ...
            */
            ...
            else
            {
                ...
                throw error(u8"此锁的所有者不是当前线程，无法进行解锁操作", error::error_type::DEBUG_thread);
            }
        }
        else
        {
            ...
            */
            ...
            throw error(u8"此锁没有上锁，无法进行解锁操作", error::error_type::DEBUG_logic);
        }
    }
    ...
    void wait_graph::erase_thread(const std::thread::id & PARAMETER_thread_id)
    {
        ...
        if (TEMP_finder == this->MEMBER_graph.end())
        {
            error TEMP_error(u8"此线程不存在于图中", error::error_type::DEBUG_bad_parameter);
            this->MEMBER_lock.store(false);
            throw TEMP_error;
        }
        else
