...
namespace OKps
{
    using TYPE_bit = unsigned int;                                                  // 直接参与位操作的类型（即整型）中，占用字节数最少的
    const constexpr std::size_t DATA_bit_byte = 8;                                  // 每字节有多少位
    const constexpr std::size_t DATA_bit_op_len = sizeof(TYPE_bit) * DATA_bit_byte; // TYPE_bit类型的位数
    using TYPE_bit_op = std::bitset<DATA_bit_op_len>;                               // 位操作的最小单位
    ...
    */
    ...
    private:
        ...
        class TYPE_pos final : public basic_no_heap
        {
        public:
            ...
            TYPE_pos(const TYPE_pos & ARG_origin) noexcept;
        };
