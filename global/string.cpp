...
namespace OKps
{
    ...
    */
    ...
    std::string string::to_string16(const size_t input)
    {
        ...
        while (temp / 16 > 0)
        {
            result[position] = string::number16_to_char(static_cast<char>(temp % 16));
            position = position - 1;
            temp = temp / 16;
        }
        result[position] = string::number16_to_char(static_cast<char>(temp));
        return result;
    }
    ...
    {
        this->MEMBER_locale = origin.MEMBER_locale;
    }
