...
/*
此文件定义一些c++内置整数类型的包装类，以杜绝内置整数类型在运算和转换时的一切未定义行为。

此库要求在c++20标准下编译，因为c++20标准规定内置整数类型都要用补码形式表示，从而规范了内置整数类型的取值范围；
c++20标准中有符号整数的负值进行位运算不再是未定义行为，而是与有符号整数非负值、无符号整数一起共享同一个定义，
该定义用四则运算描述，故其结果绝对与平台和编译器无关。至此，内置整数运算的行为终于彻底、圆满地规范了。
可以认为c++20标准下，内置整数类型只有一种未定义行为，那就是溢出，或言数学运算的结果无法用该表达式的返回类型表示。
在此基础上才能编译此文件。
