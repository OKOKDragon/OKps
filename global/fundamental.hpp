...
/*
此文件定义一些c++内置类型的包装类，以防止四则运算和位运算时的未定义行为。
在c++中，位运算的操作数必须是整数，浮点数没有位运算；浮点数的运算有大量标准库函数和浮点异常来处理，但它们的行为往往是由实现定义的。
而在c++20标准中，对很多过去是 由实现定义 的整数类型的行为作出了规范，整数的行为变得十分明确，几乎不依赖于平台和编译器。
所以此文件专注于整数。
