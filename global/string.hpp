...
namespace OKps
{
    ...
    */
    ...
    如果不能转换，则抛出异常。
    */
    char to_character(unsigned int const number, number_system const number_base = number_system::dec);
    /*
    此函数将表示数字的字符，如 '1'、'a'，转换成整数类型，不区分大小写字母
    如果不能转换，则抛出异常。
    c++标准规定，整数类型的内建算术运算符的操作数至少是int类型，char、short会被隐式转换到int再参与整数运算，故此函数返回int类型。
    */
    unsigned int from_character(char const number, number_system const number_base = number_system::dec);

    template<typename result_type>
    result_type to_number(std::string const &, number_system const number_base = number_system::dec);

    template<>
    std::size_t to_number<std::size_t>(std::string const &, number_system const number_base);
