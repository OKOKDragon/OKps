...
namespace OKps::test
{
    ...
    {
        class test_type final :public base::enable_self_copy<test_type>
        {
        private:
            std::string MEMBER_name;
        public:
            test_type(std::string const & ARG_name)noexcept
                :base::enable_self_copy<test_type>()
                , MEMBER_name(ARG_name)
            {
            }
            ~test_type()noexcept override
            {
            }
            test_type(test_type const & origin)
                :base::enable_self_copy<test_type>()
        ...
        auto obj1 = std::make_unique<test_type>("over");
        auto obj2 = obj1->self_copy();
        std::cout << dynamic_cast<test_type *>(obj2.get())->name() << "\n";
    }
    void storage_field()
    {
