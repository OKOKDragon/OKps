...
namespace OKps::aes
{
    ...
    AES 字符串加密
    */
    class string_device : public base
    {
        static_assert(sizeof(unsigned char) == sizeof(char), "在unsigned char和char大小不同的平台上没有实现");
        static_assert(sizeof(char) == 1, "char的大小不是1字节的平台上没有实现");
    public:
        string_device() = delete;
        string_device(string_device const & origin);
        void operator=(string_device const & origin);

        string_device(string_device &&) = delete;
        void operator=(string_device &&) = delete;
    private:
        string_device(const unsigned char * key) noexcept;

    public:
        string_device(const std::string & key);
    private:
        using TYPE_key = std::array<unsigned char, 16>;
        TYPE_key MEMBER_key;
    public:
        string_device(const TYPE_key & key);
    ...
    private:
        ...
        void INNER_KeyExpansion(int const i, unsigned char const rc[]) noexcept;
    ...
    private:
        std::uintmax_t MEMBER_file_size; // 文件总大小
        ...
        std::vector<std::uintmax_t> MEMBER_write_size;

        // 加密器线程数
        std::size_t cipher_number;

    public:
        std::uintmax_t file_size() const noexcept;
        const std::vector<std::uintmax_t> & write_size() const noexcept;
        std::size_t cipher_count() const noexcept;
    ...
    public:
        ...
        void operator=(file_device &&) = delete;
        ~file_device();
        ...
        key参数不能为空指针
        */
        void aes_start(const std::string & _origin_route, const std::string & _result_route, const TYPE_key & _key, const std::size_t thread_count);
        /*
        作为对外接口
        key参数的长度必须是16
        */
        void aes_start(const std::string & _origin_route, const std::string & _result_route, const std::string & _key, const std::size_t thread_count);
    ...
    private:
        ...
        public:
            aes_cipher() = delete;
            aes_cipher(TYPE_reader origin, TYPE_writter result, const unsigned long long int begin, const unsigned long long int block_number, const aes_mode mode, std::uintmax_t & write_size, const TYPE_key & key);
            aes_cipher(aes_cipher const &) = delete;
            void operator=(aes_cipher const &) = delete;
            aes_cipher(aes_cipher &&) = delete;
            void operator=(aes_cipher &&) = delete;
