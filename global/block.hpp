...
namespace OKps
{
    ...
    */
    ...
    public:
        ...
        void operator=(block const & origin)
        {
            this->data = new data_type[origin.MEMBER_size];
            this->MEMBER_size = origin.MEMBER_size;
            for (std::size_t counter = 0; counter < origin.MEMBER_size; counter++)
            {
                this->data[counter] = origin.data[counter];
            }
        }
        block(block && origin) noexcept
            :data(origin.data), MEMBER_size(origin.MEMBER_size)
        {
            origin.data = nullptr;
            origin.MEMBER_size = 0;
        }
        void operator=(block && origin) noexcept
        {
        ...
        */
        operator std::string() const
        {
            ...
            {
                result.push_back((reinterpret_cast<char *>(this->data))[counter]);
            }
            return result;
        }
        ...
        another_type & at(const size_t position)
        {
            ...
            {
                return (reinterpret_cast<another_type *>(this->data))[position];
            }
        }
        template <typename another_type>
