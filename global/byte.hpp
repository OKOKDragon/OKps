...
namespace OKps
{
    template<typename origin_type, typename target_type>
    constexpr bool const safe_convertible;

    template<>
    constexpr bool const safe_convertible<std::byte, int> = noexcept(static_cast<base::integer<int>>(std::declval<base::integer<std::underlying_type_t<std::byte>> const &>()));
    template<>
    constexpr bool const safe_convertible<std::byte, unsigned int> = noexcept(static_cast<base::integer<unsigned int>>(std::declval<base::integer<std::underlying_type_t<std::byte>> const &>()));
    template<>
    constexpr bool const safe_convertible<int, std::byte> = noexcept(static_cast<base::integer<std::underlying_type_t<std::byte>>>(std::declval<base::integer<int> const &>()));
    template<>
    constexpr bool const safe_convertible<unsigned int, std::byte> = noexcept(static_cast<base::integer<std::underlying_type_t<std::byte>>>(std::declval<base::integer<unsigned int> const &>()));
    template<>
