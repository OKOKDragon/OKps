...
发生异常的时候，程序就应该崩溃。之所以先抛出异常，而不是直接立刻崩溃，只是为了让程序记录崩溃日志或者执行一些清理代码，
不是为了让程序强行处理它不该处理的错误，并继续带病运行下去的。
对于这样的错误，就是应该使用异常，而不是任何其他机制。
对于可恢复性的业务逻辑错误，再去考虑用错误码或其他机制。

此库中所有函数在抛出异常前，会设施全局本地环境为初始值，即调用{std::locale::global(std::locale::classic());}，以确保错误信息可以正确输出到标准输出。
这些异常的错误信息都是编译期确定的字符串常量。
*/
