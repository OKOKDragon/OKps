...
namespace OKps
{
    ...
    integer::known_prime_holder::known_prime_holder()
        ...
        and noexcept(std::is_nothrow_default_constructible_v<std::once_flag>))
    {
    }
    integer::known_prime_holder::~known_prime_holder()
        noexcept(std::is_nothrow_destructible_v<std::vector<integer>>
        and std::is_nothrow_destructible_v<std::once_flag>)
    {
    }
    integer const & integer::known_prime_holder::operator [](std::size_t const position)const
    {
        std::call_once(this->MEMBER_flag, &known_prime_holder::INNER_init_known_prime, this->MEMBER_known_prime);
        if (position >= this->MEMBER_known_prime.size())
        {
            throw error(std::out_of_range("访问已知素数表时越界"));
        }
    ...
    void integer::known_prime_holder::INNER_init_known_prime(std::vector<integer> & holder)
    {
