...
namespace OKps
{
    ...
    integer::integer(holder_type const number, sign_type const sign)noexcept
        :MEMBER_sign(sign)
    {
        if (sign == sign_type::zero)
        {
    ...
    std::vector<integer> const integer::MEMBER_known_prime = INNER_init_known_prime();

    std::vector<integer::holder_type> integer::INNER_get_known_prime()
    {
        ...
        {
            std::string const hint = "程序初始化错误 ││ 无法打开已知素数文件（"
        ...
        for (std::size_t i = 0;i < length;++i)
        {
            ...
            {
                std::string const hint = "程序初始化错误 ││ 无法读取已知素数文件（"
                    + std::filesystem::absolute(prime_file_route).string()
                    + "）。";
                std::cout << hint << "\n";
                OKps::wait_input("");
    ...
    std::vector<integer> integer::INNER_init_known_prime()
    {
        auto const TEMP_known_prime = integer::INNER_get_known_prime();
