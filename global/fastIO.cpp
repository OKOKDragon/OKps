...
namespace OKps::file
{
    ...
    void manager::INNER_check(std::filesystem::path const & file, manager::state const op)
    {
        manager::MEMBER_lock.lock();
        auto finder = manager::MEMBER_pool.find(file);
        if (finder == manager::MEMBER_pool.end())
        {
            manager::MEMBER_pool.insert(std::make_pair(file, std::make_pair(op, 1)));
        }
        else
        {
            if (finder->second.first != op)
            {
                if (finder->second.first == manager::state::reading)
                {
                    std::logic_error hint(std::string("文件") + file.string() + "正在读取，禁止写入");
                    manager::MEMBER_lock.unlock();
                    throw hint;
                }
                else
                {
                    std::logic_error hint(std::string("文件") + file.string() + "正在写入，禁止读取");
                    manager::MEMBER_lock.unlock();
                    throw hint;
                }
            }
            ...
            {
                std::logic_error hint(std::string("文件") + file.string() + "正在写入，禁止另一个写入");
                manager::MEMBER_lock.unlock();
                throw hint;
            }
        }
    ...
    void manager::INNER_erase(std::filesystem::path const & file)
    {
        manager::MEMBER_lock.lock();
        auto finder = manager::MEMBER_pool.find(file);
        if (finder->second.second == 1)
        {
            manager::MEMBER_pool.erase(finder);
        }
        else
        {
            finder->second.second--;
        ...
        : buffer_size(size), end_position(0), file_size(0), read_position(0), head_position(0), buffer(new char[size])
    {
        ...
        {
            this->buffer[TEMP_count] = static_cast<char>(0);
        }
    }
    ...
    reader::~reader()
    {
        ...
        {
            this->file.close();
            manager::INNER_erase(this->route);
        }
    }
    ...
    void reader::INNER_open(string_type _route, std::ios_base::openmode _mode, int _prot, bool resize)
    {
        if (this->file.is_open())
        {
            throw std::logic_error("已经打开文件，禁止重复打开");
        }
        else
        {
            this->route = _route;
            ...
            if (this->file.is_open())
            {
                manager::INNER_check(this->route, manager::state::reading);
                // 根据输入的文件打开模式，调整读指针
                ...
                {
                    this->file.close();
                    throw std::invalid_argument("_mode参数不合法");
                }
            }
        }
    }
    ...
    void reader::close()
    {
        ...
        {
            throw std::logic_error("没有打开的文件，无法关闭文件");
        }
    }

    std::streampos reader::tellg() const
    {
        if (not this->file.is_open())
        {
            throw std::logic_error("没有打开文件，无法访问文件指针");
        }
    ...
    void reader::seekg(std::streamoff _off, std::ios_base::seekdir _way)
    {
        ...
        else
        {
            ...
            throw std::invalid_argument("_way 参数不合法");
        }
    }
    bool reader::read(std::unique_ptr<char[]> const & read_buffer, std::streamsize size)
    {
        return this->read(read_buffer.get(), size);
    }
    bool reader::read(std::shared_ptr<char[]> const & read_buffer, std::streamsize size)
    {
        return this->read(read_buffer.get(), size);
    }
    bool reader::read(char * read_buffer, std::streamsize size)
    {
        ...
        if (not this->file.is_open())
        {
            throw std::logic_error("没有打开文件，无法读取");
        }
        if (size <= 0)
        {
            return true;
        ...
        while (true)
        {
            ...
            if (this->read_position < this->head_position or this->read_position >= (this->head_position + static_cast<std::streamoff>(this->end_position)))
            {
            ...
            if (this->read_position + size <= this->head_position + static_cast<std::streamoff>(this->end_position))
            {

                // 要读的数据完全在缓冲区内
                for (std::size_t count = 0; count < size; count++)
                {
                    read_buffer[count] = this->buffer[this->read_position + static_cast<std::streamoff>(count) - this->head_position];
                }
            ...
            else
            {
                ...
                // 先把缓冲区的相应数据全部写入
                auto to_read_size = this->head_position + static_cast<std::streamoff>(this->end_position) - this->read_position;
    ...
    void reader::refresh_buffer()
    {
        if (this->read_position >= 0 and this->read_position <= this->file_size)
        {
            ...
            {
                // 把文件读完，填不满缓冲区
                if (this->file.read(this->buffer.get(), to_read_size))
                {
                }
                else
                {
            ...
            {
                // 填满缓冲区
                if (this->file.read(this->buffer.get(), this->buffer_size))
                {
                }
                else
                {
        ...
        else
        {
            ...
            throw std::out_of_range("申请的读指针位置超出文件范围");
        }
        /*
        读真实文件时出错
        */
    FLOW_read_error:
        {
            ...
            throw std::runtime_error(std::string("读文件") + this->route.string() + "失败");
        }
    }
    ...
    std::uintmax_t reader::size() const
    {
        if (not this->file.is_open())
        {
            throw std::logic_error("没有打开文件，无法获取大小");
        }
    ...
    template <typename string_type>
    void writter::INNER_open(string_type _route, std::ios_base::openmode _mode, int _prot, bool resize, std::uintmax_t guess_size)
    {
        if (this->file.is_open())
        {

            throw std::logic_error("已经有一个打开的文件，禁止重复打开");
        }
        else
        {
            this->route = _route;
            ...
            if (this->file.is_open())
            {
                manager::INNER_check(this->route, manager::state::writting);
                // 根据输入的文件打开模式，调整写指针
    ...
    void writter::open(const std::string & _route, std::ios_base::openmode _mode, int _prot, bool resize, std::uintmax_t guess_size)
    {
        this->INNER_open(_route, _mode, _prot, resize, guess_size);
    }

    void writter::open(const char * _route, std::ios_base::openmode _mode, int _prot, bool resize, std::uintmax_t guess_size)
    {
        this->INNER_open(_route, _mode, _prot, resize, guess_size);
    }

    void writter::open(const std::wstring & _route, std::ios_base::openmode _mode, int _prot, bool resize, std::uintmax_t guess_size)
    {
        this->INNER_open(_route, _mode, _prot, resize, guess_size);
    }

    void writter::open(const wchar_t * _route, std::ios_base::openmode _mode, int _prot, bool resize, std::uintmax_t guess_size)
    {
        this->INNER_open(_route, _mode, _prot, resize, guess_size);
    }

    void writter::open(const std::filesystem::path & _route, std::ios_base::openmode _mode, int _prot, bool resize, std::uintmax_t guess_size)
    {
        this->INNER_open(_route, _mode, _prot, resize, guess_size);
    }
    ...
    std::streampos writter::tellp() const
    {
        if (not this->file.is_open())
        {
            throw std::logic_error("没有打开文件，无法访问文件指针");
    ...
    void writter::seekp(std::streamoff _off, std::ios_base::seekdir _way)
    {
        ...
        else
        {
            ...
            throw std::invalid_argument("不合法的参数 _way");
        }
    }
    ...
    bool writter::write(const char * write_buffer, std::streamsize size)
    {
        ...
        if (not this->file.is_open())
        {
            throw std::logic_error("没有打开文件，无法写入");
        }
        if (size <= 0)
        {
            return true;
        ...
        while (true)
        {
            if (this->write_position < this->head_position or this->write_position >= this->head_position + static_cast<std::streamoff>(this->buffer_size))
            {
                // 写指针在缓冲区的有效范围之外
                // 刷新缓冲区
                this->refresh_buffer();
            }
            if (this->write_position + size <= this->head_position + static_cast<std::streamoff>(this->buffer_size))
            {
                // 要写的内容能全部塞进缓冲区
                for (std::size_t count = 0; count < size; count++)
                {
            ...
            else
            {
                ...
                this->write_position = this->write_position + static_cast<std::streamoff>(to_write_size);
    ...
    void writter::resize_buffer(const std::streamoff size)
    {
        if (size <= 0)
        {
            throw std::invalid_argument("缓冲区长度必须为正数");
        }
        ...
        if (size < used_size and this->file.is_open())
        {
            // 新缓冲区的长度 < 旧缓冲区记录的有效数据的长度
            // 把塞不下的这部分数据写入文件
            std::size_t to_write_size = used_size - size;
            auto TEMP_valid_itor = this->valid.cbegin() + size;
            this->valid_writter(&(this->buffer[size]), TEMP_valid_itor, to_write_size, this->head_position + size);
        }
    ...
    void writter::refresh_buffer()
    {
        ...
        this->valid_writter(this->buffer.get(), TEMP_valid_itor, this->buffer_size, this->head_position);
    ...
    void writter::auto_resizer(const std::uintmax_t size_guess)
    {
    ...
    */
    bool writter::write(std::unique_ptr<char const []> const & write_buffer, std::streamsize size)
    {
        return this->write(write_buffer.get(), size);
    }
    bool writter::write(std::shared_ptr<char const []> const & write_buffer, std::streamsize size)
    {
        return this->write(write_buffer.get(), size);
    }
    bool end_writter::write(std::unique_ptr<char const []> const & write_buffer, std::streamsize size)
    {
        return this->write(write_buffer.get(), size);
    }
    bool end_writter::write(std::shared_ptr<char const []> const & write_buffer, std::streamsize size)
    {
        return this->write(write_buffer.get(), size);
    }
    bool writter::write(std::string const & content)
    {
        return this->write(content.c_str(), content.size());
    }
    bool end_writter::write(std::string const & content)
    {
        return this->write(content.c_str(), content.size());
    }
    void writter::valid_writter(const char * write_buffer, const std::vector<bool>::const_iterator & checker, const std::size_t length, std::streampos _pos)
    {
        ...
        while (end < last)
        {
            ...
            else
            {
                ...
                throw std::runtime_error(std::string("文件") + this->route.string() + "写入失败");
            }
        }
    }

    end_writter::end_writter(std::size_t size)
        : buffer_size(size), buffer(new char[size]), file_size(0), write_position(0), head_position(0), app_mode(false)
    {
        // 把数据全部初始化，是为了真正占有申请的内存
        // 现代操作系统，只有在进程尝试使用内存的时候，才会真正分配给进程
    ...
    template <typename string_type>
    void end_writter::INNER_open(string_type _route, std::ios_base::openmode _mode, int _prot, bool resize, std::uintmax_t guess_size)
    {
        if (this->file.is_open())
        {
            throw std::logic_error("已经有一个打开的文件，禁止重复打开");
    ...
    void end_writter::open(const std::string & _route, std::ios_base::openmode _mode, int _prot, bool resize, std::uintmax_t guess_size)
    {
        this->INNER_open(_route, _mode, _prot, resize, guess_size);
    }

    void end_writter::open(const char * _route, std::ios_base::openmode _mode, int _prot, bool resize, std::uintmax_t guess_size)
    {
        this->INNER_open(_route, _mode, _prot, resize, guess_size);
    }

    void end_writter::open(const wchar_t * _route, std::ios_base::openmode _mode, int _prot, bool resize, std::uintmax_t guess_size)
    {
        this->INNER_open(_route, _mode, _prot, resize, guess_size);
    }

    void end_writter::open(const std::wstring & _route, std::ios_base::openmode _mode, int _prot, bool resize, std::uintmax_t guess_size)
    {
        this->INNER_open(_route, _mode, _prot, resize, guess_size);
    }

    void end_writter::open(const std::filesystem::path & _route, std::ios_base::openmode _mode, int _prot, bool resize, std::uintmax_t guess_size)
    {
        this->INNER_open(_route, _mode, _prot, resize, guess_size);
    }

    bool end_writter::is_open() const noexcept
    {
        return this->file.is_open();
    }

    void end_writter::close()
    {
        if (this->file.is_open())
        {
        ...
        {
            throw std::logic_error("没有打开文件，无法关闭文件");
        }
    }

    std::streampos end_writter::tellp() const
    {
        if (not this->file.is_open())
        {
            throw std::logic_error("没有打开文件，无法返回文件指针");
        }
        return this->write_position;
    }

    bool end_writter::write(const char * write_buffer, std::streamsize size)
    {
        if (write_buffer == nullptr)
        {
            throw std::invalid_argument("禁止输入空指针");
    ...
    void end_writter::resize_buffer(std::streamoff const size)
    {
        if (size <= 0)
        {
            throw std::invalid_argument("缓冲区大小必须是正数");
        }
        ...
        if (size < used_size and this->file.is_open())
        {
            ...
            this->file.seekp(this->head_position + size);
    ...
    void end_writter::refresh_buffer()
    {
        // 把缓冲区内的数据写入文件，然后清空缓冲区
        this->file.seekp(this->head_position);
        if (this->file.write(this->buffer.get(), this->write_position - this->head_position))
        {
        }
        else
        {
    ...
    void end_writter::auto_resizer(std::uintmax_t size_guess)
    {
        if (size_guess <= 1000000)
        {
            this->resize_buffer(size_guess);
