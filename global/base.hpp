...
注意，有多层虚继承时须手动在最终派生类的构造函数中调用虚基类的构造函数。
例如，设基类A被类B虚继承，而类C又虚继承自类B，则在类C的各个构造函数中，都应显式调用基类A构造函数，而不能只调用类B的构造函数，
因为无论类B的构造函数中调用了基类A的哪一种构造函数，类C的构造函数都只会隐式调用基类A的默认构造函数。
*/
