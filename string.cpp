    ...
    */
    ...
    std::string string::Unicode_to_UTF8(const std::wstring & origin)
    {
        ...
        try
        {
            ...
            result = temp.to_bytes(origin);
        }
        catch (const std::exception & error_info)
        {

            error OKps_error_info(u8"Unicode编码向UTF8编码转换时，发生错误", error::error_type::CPP_build_in);
            throw bind_error<std::exception>(OKps_error_info, error_info);
        }
        return result;
    }
    ...
    std::wstring string::UTF8_to_Unicode(const std::string & origin)
    {
        ...
        {
            std::wstring_convert<std::codecvt_utf8<wchar_t>> temp;
            result = temp.from_bytes(origin);
        }
        catch (const std::exception & error_info)
        {
            error OKps_error_info(u8"UTF8编码向Unicode编码转换时，发生错误", error::error_type::CPP_build_in);
            throw bind_error<std::exception>(OKps_error_info, error_info);
        }
        return result;
    }
    ...
    char string::to_char(const char input)
    {
        ...
        {
            throw error(u8"输入的不是10进制数字", error::error_type::DEBUG_bad_parameter);
        }
    }
    ...
    size_t string::string16_to_ULL(const std::string & input)
    {
        if (input.size() != 2 * (sizeof(size_t) / sizeof(char)))
        {
            ...
            throw error(u8"输入串长度错误", error::error_type::DEBUG_bad_parameter);
        }
        size_t result = 0;
        for (size_t count = 0; count < 2 * (sizeof(size_t) / sizeof(char)); count++)
        {
    ...
    char string::number16_to_char(const char input)
    {
        if (input < 0 or input > 15)
        {
            ...
            throw error(u8"输入的不是16进制数字", error::error_type::DEBUG_bad_parameter);
        }
        if (input >= 0 and input <= 9)
        {
            return (input + '0');
    ...
    char string::char16_to_number(const char input)
    {
        ...
        else
        {
            ...
            throw error(u8"输入的字符不是16进制数字", error::error_type::DEBUG_bad_parameter);
        }
    }
